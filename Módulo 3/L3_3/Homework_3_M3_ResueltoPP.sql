...
WHERE 	(FLOOR(Cnt/2) = CEILING(Cnt/2) AND (RowNum = FLOOR(Cnt/2) OR RowNum = FLOOR(Cnt/2) + 1)) -- par
	OR
		(FLOOR(Cnt/2) <> CEILING(Cnt/2) AND RowNum = CEILING(Cnt/2))  -- Impar
GROUP BY ProductID;
...
/* Con ERROR de agrupamiento por columnas calculadas
SELECT ProductID, 
...
FROM (
	SELECT	d.ProductID,
			...
			COUNT(*) OVER (PARTITION BY d.ProductID) AS Cnt,
			ROW_NUMBER() OVER (PARTITION BY d.ProductID ORDER BY d.LineTotal) AS RowNum,
            AVG(d.LineTotal) OVER (PARTITION BY d.ProductID) AS Media
	FROM	salesorderheader h
		JOIN salesorderdetail d ON (h.SalesOrderID = d.SalesOrderID)) v
WHERE 	(FLOOR(Cnt/2) = CEILING(Cnt/2) AND (RowNum = FLOOR(Cnt/2) OR RowNum = FLOOR(Cnt/2) + 1)) -- par
	OR
...
WHERE   (FLOOR(Cnt/2) = CEILING(Cnt/2) AND (RowNum = FLOOR(Cnt/2) OR RowNum = FLOOR(Cnt/2) + 1)) -- par
    OR  (FLOOR(Cnt/2) <> CEILING(Cnt/2) AND RowNum = CEILING(Cnt/2))  -- Impar
GROUP BY ProductID, Cnt, Media;

-- Funcionando sin agrupar por todas las columnas calculadas
SELECT ProductID, 
...
SELECT ProductID, 
	-- AVG(LineTotal) OVER (partition by ProductID) AS Media, 
    AVG(LineTotal), 
    Count(*) OVER (partition by ProductID) As Conteo
FROM salesorderdetail
GROUP BY productID;
