...
ALTER TABLE `producto` ADD COLUMN `Concepto` VARCHAR(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_spanish_ci NULL DEFAULT NULL;
...
UPDATE empleado SET Salario2 = replace(Salario2, ',', '.');
...
/* DROP FUNCTION UC_Words;
DELIMITER $$
CREATE FUNCTION UC_Words(cadena varchar(150)) returns varchar(150)
    BEGIN
    DECLARE len INT;
    DECLARE i INT;
...
UPDATE cliente SET  Domicilio = UC_Words(Domicilio),
                    Nombre_y_Apellido = UC_Words(Nombre_y_Apellido);

SELECT * FROM cliente;
					
UPDATE sucursal SET Domicilio = UC_Words(Domicilio),
                    Sucursal = UC_Words(Sucursal);
