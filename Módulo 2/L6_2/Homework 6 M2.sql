/* Ejercicio 1. tuve que modificar el nombre de algunos campos para adaptarlos a la base de
datos que había creado.*/

USE henry;
ALTER TABLE Alumno rename alumno;
ALTER TABLE Carrera rename carrera;
ALTER TABLE Cohorte rename cohorte;
ALTER TABLE Instructor rename instructor;
...
ALTER TABLE `henry`.`instructor` 
CHANGE COLUMN `cedula_identidad` `cedulaIdentidad` INT NOT NULL ,
CHANGE COLUMN `nombre_instructor` `nombre` VARCHAR(20) NOT NULL ,
CHANGE COLUMN `apellido_instructor` `apellido` VARCHAR(20) NOT NULL ,
CHANGE COLUMN `fecha_nacimiento` `fechaNacimiento` DATE NOT NULL ,
CHANGE COLUMN `fecha_incorporacion` `fechaIncorporacion` DATE NULL;
...
ALTER TABLE cohorte MODIFY COLUMN instructor INT AFTER carrera;
...
INSERT INTO cohorte (codigo, carrera, instructor, fecha_inicio, fecha_fin)
VALUES ('FT-1235',1,1,'2020-2-1','2020-6-30');

select codigo from cohorte;
...
CREATE TABLE alumno (
    ...
    idCohorte INT,
    FOREIGN KEY (idCohorte) REFERENCES cohorte(id_cohorte)
);
...
-- Ejercicio 6
SELECT i.cedulaIdentidad, CONCAT(i.nombre," ", i.apellido) AS nombre_completo, c.codigo
FROM instructor i
JOIN cohorte c
ON i.id_instructor = c.instructor
WHERE c.codigo LIKE 'FT%'; 

SELECT i.cedulaIdentidad, CONCAT(i.nombre," ", i.apellido) AS nombre_completo, car.nombre
FROM instructor i
JOIN cohorte c
ON i.id_instructor = c.instructor
JOIN carrera car
ON c.carrera = car.id_carrera
WHERE car.nombre LIKE 'Full%'; 

-- Ejercicio 7
CREATE VIEW alumnos_1235 AS
SELECT idAlumno, nombre, apellido, cedulaIdentidad 
FROM alumno
...
ON a.idCohorte=c.id_cohorte
INNER JOIN carrera car
ON c.carrera = car.id_carrera;

/*
Los campos que permiten acceder al nombre de carrera son cohorte.carrera, carrera.id_carrera
y carrera.nombre .
La cardinalidad entre la tabla cohorte y alumno es: uno a muchos.
*/
-- c.
SELECT a.nombre, a.apellido, a.fechaNacimiento, car.nombre
FROM alumno a
...
INNER JOIN carrera car
ON c.carrera = car.id_carrera
WHERE car.nombre NOT LIKE 'Data%';

/* La mejor opción para hacer este listado es utilizar LIKE, porque define 
lo que buscamos. Si utilizamos NOT LIKE, a futuro puede cambiar el resultado si se 
incorporan más carreras. */
...
INNER JOIN carrera car
ON c.carrera = car.id_carrera
WHERE car.nombre != 'Data Science';

/* La mejor opción para hacer este listado es utilizar =, porque define 
lo que buscamos. Si utilizamos !=, a futuro puede cambiar el resultado si se 
incorporan más carreras. 

e. Para utilizar el campo idCarrera en el filtrado todo el SELECT es el mismo
sólo cambia el filtro WHERE donde en lugar de filtrar por carrera.nombre, se puede 
utilizar carrera.id_carrera o cohorte.carrera. Y en lugar de hacerlo por el nombre de la
carrera se deben definir los parámetros por el id de la carrera.*/